...
async fn test_websocket_echo() {
    ...
    ws_stream.send(tungstenite::protocol::Message::Text(msg.into())).await.expect("Failed to send message");

    // Receive the echoed message.
    let response = ws_stream.next().await.expect("No message received").expect("Failed to receive message");
